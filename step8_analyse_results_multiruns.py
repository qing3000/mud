...
imageNum6s, cribNum6s, row6s, col6s, v6s, temp, labels = np.loadtxt('Output\\RioTinto_Clean_result.csv', delimiter = ',', skiprows = 1, unpack = True)
imageNum7s, cribNum7s, row7s, col7s, v7s, temp, labels = np.loadtxt('Output\\RioTinto_Muddy_result.csv', delimiter = ',', skiprows = 1, unpack = True)
...
plt.plot(hx6, hy6, label = 'RioTinto All Cribs (%d samples)' % len(imageNum6s))
plt.xlim([-50, 20])
#plt.plot(hx7, hy7, label = 'RioTinto Muddy (%d samples)' % len(imageNum7s))
plt.grid(True)
plt.legend(loc = 0)
...
plt.plot(hx5, hy5, label = 'Run 364 batch 3(%d samples)' % len(imageNum5s))
plt.plot(hx6, hy6, label = 'RioTinto All Cribs (%d samples)' % len(imageNum6s))
#plt.plot(hx7, hy7, label = 'RioTinto Clean (%d samples)' % len(imageNum7s))
plt.xlim([-10, 10])
plt.ylim([0, 0.04])
plt.grid(True)
