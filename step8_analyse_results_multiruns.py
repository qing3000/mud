...
def CopyImagesByRange_RioTinto(rngs, vs, fns):
    indices = np.nonzero(np.logical_and(vs > rngs[0], vs < rngs[1]))[0]
    for i in indices[:500]:
        im = imread('Output\\Blocks\\RioTinto\\Muddy\\%s.png' % fns[i])
        foutPath = 'Diagnostics\\RioTinto\\%03d-%03d' % (rngs[0], rngs[1])
        if not os.path.exists(foutPath):
            os.mkdir(foutPath)
        imwrite('%s\\%s.jpg' % (foutPath, fns[i]), im)
...
for r in range(-30, 10, 5):
    CopyImagesByRange_RioTinto((r, r + 5), v7s, fn7s)
