...
def CopyImagesByRange(runNum, r, vs, imageNums, cribNums, rows, cols):
    for i in np.nonzero(np.logical_and(vs > r, vs < r + 0.01))[0][:3]:
        fn = 'Run%s_Image%05d_Crib%04d_Row%d_Col%d' % (runNum, imageNums[i], cribNums[i], rows[i], cols[i])
...
imageNum1s, cribNum1s, v1s, temp, labels = np.loadtxt('Output\\Run132_Cribs_result.csv', delimiter = ',', skiprows = 1, unpack = True)
imageNum2s, cribNum2s, v2s, temp, labels = np.loadtxt('Output\\Run354_Cribs_result.csv', delimiter = ',', skiprows = 1, unpack = True)
imageNum3s, cribNum3s, v3s, temp, labels = np.loadtxt('Output\\Run364_Batch1_Cribs_result.csv', delimiter = ',', skiprows = 1, unpack = True)
imageNum4s, cribNum4s, v4s, temp, labels = np.loadtxt('Output\\Run364_Batch2_Cribs_result.csv', delimiter = ',', skiprows = 1, unpack = True)
imageNum5s, cribNum5s, v5s, temp, labels = np.loadtxt('Output\\Run364_Batch3_Cribs_result.csv', delimiter = ',', skiprows = 1, unpack = True)
fn6s, v6s, temp, labels = np.genfromtxt('Output\\RioTinto_Cribs_All_result.csv', delimiter = ',', skip_header = 1, unpack = True)
fn7s, v7s, temp, labels = np.genfromtxt('Output\\RioTinto_Cribs_Muddy_result.csv', delimiter = ',', skip_header = 1, unpack = True)
fn6s = np.genfromtxt('Output\\RioTinto_Cribs_All_result.csv', delimiter = ',', skip_header = 1, dtype = str, usecols = 0)
fn7s = np.genfromtxt('Output\\RioTinto_Cribs_Muddy_result.csv', delimiter = ',', skip_header = 1, dtype = str, usecols = 0)
...
plt.grid(True)
plt.legend(loc = 0)
plt.title('Training Data Distributions (Cribs)')         
plt.subplot(3,1,2)
...
plt.legend(loc = 0)
plt.grid(True)
plt.title('Kiwi ARC and Vline Distributions (Cribs)')       
...
plt.legend(loc = 0)
plt.grid(True)
plt.title('Rio Tinto Distributions (Cribs)')         
