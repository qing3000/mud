...
def main():
    ...
    cleanFns = glob('..\\AMTrakTiles\\Clean\\*.png', recursive = True)[:-800]
    # shuffle(cleanFns)
    # cleanFns = cleanFns[:5000]
    muddyFns = glob('..\\AMTrakTiles\\Mud\\*.png', recursive = True)[:-800]
    ...
    print('Prepare 2 batches for training')
    # subset_images = train_images[:3000]
    # subset_labels = train_labels[:3000]
    print('Train the CNN batch 1')
    history = model.fit(np.array(train_images), train_labels, validation_split = 0.3, epochs = 10)
    print('Train the CNN batch 2')
    # subset_images = np.array(train_images[3000:])
    # subset_labels = train_labels[3000:]
    # history2 = model.fit(subset2_images, subset2_labels, epochs = 10)
    ...
    np.save('TrainingHistory_tiles.npy', history.history)
