...
from step6_training_fft import shuffle_filenames_and_labels, read_image
...
if __name__ == '__main__':  
    ...
    model = models.load_model('CNN_Model_FFT')
    ...
    f = open('Output\\Clean_FFT_values.csv', 'w')
    f.write('\n'.join(map(str, clean_values)))
    f.close()
    f = open('Output\\Muddy_FFT_values.csv', 'w')
    f.write('\n'.join(map(str, muddy_values)))
    f.close()

    clean_values = np.loadtxt('Output\\Clean_FFT_values.csv')
    muddy_values = np.loadtxt('Output\\Muddy_FFT_values.csv')
    ...
    plt.legend(loc = 0)
    plt.grid(True)
    plt.title('Network output distributions LDA =%g (FFT images)' % lda)
